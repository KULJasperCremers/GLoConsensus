...
def plot_global_sm_and_induced_paths(
    timeseries_list, global_similarity_matrix, induced_paths, representative
):
    fig, axs, _ = plot_sm(
    ...
    axs = plot_local_warping_paths(axs, induced_paths, lw=5)
    axs[3].axvline(representative[0], lw=1, c='k', ls='--')
    axs[3].axvline(representative[1], lw=1, c='k', ls='--')
    fig.savefig(
        f'./plots/global_sm_with_ips_({representative[0]},{representative[1]}).png'
    )
    plt.close(fig)
